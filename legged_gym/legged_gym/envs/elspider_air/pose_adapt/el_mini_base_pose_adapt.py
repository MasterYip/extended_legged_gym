...
class ElMiniBasePoseAdaptCfg(BasePoseAdaptCfg):

    # Randomization options
    randomize_init_pos: bool = True
    randomize_init_yaw: bool = True

    class env(BasePoseAdaptCfg.env):
    ...
    class asset(BasePoseAdaptCfg.asset):
        file: str = "{LEGGED_GYM_ROOT_DIR}/resources/robots/el_mini/urdf/el_mini_base.urdf"
        name: str = "el_mini_base"
        flip_visual_attachments = False  # Some .obj meshes must be flipped from y-up to z-up
        # Default nominal height for the robot base
        nominal_height: float = 0.2  # Appropriate height for ElSpider Air [m]
    ...
    class commands(BasePoseAdaptCfg.commands):
        # Command configuration for ElSpider Air
    ...
    class rewards(BasePoseAdaptCfg.rewards):
        ...
        lin_vel_tracking: float = 0.008
        ang_vel_tracking: float = 0.005

        # Downward velocity reward
        downward_vel_reward: float = 0.0025  # Higher weight for encouraging downward movement
...
class ElMiniBasePoseAdaptCfgPPO(BasePoseAdaptCfgPPO):
    """PPO configuration parameters specifically for ElSpider Air base pose adapt task."""

    seed: int = 1
    runner_class_name = "OnPolicyRunner"

    class algorithm(BasePoseAdaptCfgPPO.algorithm):
        # Additional/overridden PPO algorithm parameters for ElSpider Air
...
class ElMiniBasePoseAdapt(BasePoseAdapt):
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        # Initialize the base class
        super().__init__(cfg, sim_params, physics_engine, sim_device, headless)
        # Set ElSpider Air specific parameters - nominal_height is now from config
